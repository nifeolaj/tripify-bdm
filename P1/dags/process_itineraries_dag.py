...
def extract_city(filename):
    match = re.search(r'itinerary_([a-zA-Z]+)(?:_\d+)?_[^\.]+\.(?:txt|csv|json)$', filename)
    if match:
...
def process_files(ti, bucket_name):
    ...
    for filename in files:
        ...
        destination_blob_name = f"landing_zone/llm_itineraries/{city}/{date_prefix}/{file_name}"
...
with DAG(
    'gcs_itineraries_processing',
    start_date=datetime(2025, 3, 29),
    description='DAG to process itineraries data to persistent storage',
    tags=['gcs_itineraries_processing'],
    schedule='@daily',
    catchup=False
) as dag:
    ...
    check_prefix = GCSObjectsWithPrefixExistenceSensor(
        ...
        prefix=f'tmp_landing_zone/llm/{date_prefix}',
        poke_interval=60,
        timeout=3600,
        mode='reschedule',
        soft_fail=True
    ...
    dag.dataset = Dataset(f"gs://{gcs_bucket_name}/tmp_landing_zone/llm/{generate_date()}")